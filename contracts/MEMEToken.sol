...
contract MetaNodeToken is ERC20Burnable {
    ...
    /**
     * @notice 转账/铸造/销毁统一钩子 - 实现转账限制逻辑
     * @dev 重写 ERC20 的 _update（OZ v5 替代原 _beforeTokenTransfer）
     *
     * @param from 发送方地址（铸造时为 address(0)）
     * @param to 接收方地址（销毁时为 address(0)）
     * @param value 数量
     *
     * ============ 检查规则 ============
     *
