    ...
    ReentrancyGuardUpgradeable
{
     ...
     * 每个受益人可以有多个独立的归属计划
     */
    mapping(address => mapping(address => mapping(uint256 => VestingSchedule)))
    ...
    function initialize(address _admin, address _operator) public initializer {
        ...
        // UUPSUpgradeable (OZ v5) 无状态，无需 __UUPSUpgradeable_init()
                ...
                if (block.timestamp >= schedule.endTime) {
                    ...
                    if (schedule.mode == VestingMode.LINEAR) {
                        uint256 timePassed = block.timestamp -
                            schedule.startTime;
                        uint256 totalDuration = schedule.endTime -
                            schedule.startTime;
                        vestedForSchedule =
                            (schedule.totalAmount * timePassed) /
                            totalDuration;
